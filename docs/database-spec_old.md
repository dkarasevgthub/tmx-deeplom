# ProЗапас — техническое задание на базу данных (устарело)

> **Документ устарел.** Актуальный состав схемы и границы сервисов —
> [database-architecture.md](database-architecture.md), там же в разделе 3.2
> перечислено, что изменилось. Этот файл сохранён как история решений; при
> расхождении верен `database-architecture.md`.

Схема PostgreSQL для складской системы. Работает в паре с
[backend-spec_old.md](backend-spec_old.md); архитектурные обоснования —
